...
class PackTab(Tab):
    ...
    def on_load(self):
        #for i in range(50):
            #ttk.Checkbutton(self.frame.scrollable_frame, text=str("lol"), variable=i).grid(row=i, column=0, pady=5, padx=5, sticky="nswe")
        #self.list_box.delete(0, tk.END)
        i = 0
        for exe in self.get_packs():
            from tkinter import font
            self.item = tk.Checkbutton(self.frame.scrollable_frame, text=str(exe), font=("Segoe UI",18,"bold"), variable=i)
            self.item.grid(row=i, column=1, pady=5, padx=5, sticky="nswe")
            path = cwd + "/noicon.png"
            self.img = ImageTk.PhotoImage(Image.open(path))
            self.imgpanel = tk.Label(self.frame.scrollable_frame, image = self.img, width=50, height=50)
            self.imgpanel.grid(row=i, column=0, pady=5, padx=5, sticky="nswe")
