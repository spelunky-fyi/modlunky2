...
use ml2_mods::{
    ...
    spelunkyfyi::{
        http::{DEFAULT_SERVICE_ROOT, HttpApiMods},
        web_socket::{
...
struct Cli {
    ...
    // Poll and scan more aggressively to make the demo snappier
...
async fn main() -> anyhow::Result<()> {
    ...
    let api_client = cli
        .token
        .as_ref()
        .map(|token| HttpApiMods::new(&cli.service_root, token, reqwest::Client::new()))
        .transpose()?;
    ...
    Toplevel::new(async move |s: &mut SubsystemHandle| {
        ...
        if let Some(web_socket_client) = web_socket_client {
            s.start(SubsystemBuilder::new(
                "WebSocket",
                web_socket_client.into_subsystem(),
            ));
        }
        s.start(SubsystemBuilder::new(
            "CLI",
            async move |h: &mut SubsystemHandle| {
                run(h, cli.command, manager_handle, mod_cache_handle, changes_rx).await
            },
        ));
    })
    .catch_signals()
