...
use ml2_mods::{
    ...
    local::{
        Error as LocalError,
        constants::{MANIFEST_FILENAME, MOD_METADATA_SUBPATH, MODS_SUBPATH},
        disk::DiskMods,
    },
    manager::{DEFAULT_RECEIVING_INTERVAL, Error, ModManager, ModManagerHandle, ModSource},
...
fn setup(install_path: &str) -> ModManagerHandle {
    ...
    // These are "dangling" because the test doesn't wire up a full system
...
async fn assert_missing(dir: &TempDir, mod_id: &str, path: &str) {
    let path = dir.path().join(MODS_SUBPATH).join(mod_id).join(path);
    match fs::metadata(&path).await {
        Err(e) if e.kind() == io::ErrorKind::NotFound => (),
        Ok(_) => panic!("Expected {path:?} to have been removed by update"),
        Err(e) => panic!("Unexpected IO error checking {path:?}: {e:?}"),
    }
}

/// Playlunky writes savegame data into the mod's own folder. Updating a mod
/// (reinstalling in place) must preserve those files or a user's save is
/// silently lost.
#[tokio::test]
async fn test_update_preserves_savegames() {
    let dir = tempfile::tempdir().unwrap();
    let handle = setup(dir.path().to_str().unwrap());

    let mod_id = "savegame-mod";
    install_from_local_sources(&handle, "single_dir.zip", mod_id).await;
    assert_exits_in(&dir, mod_id, "foo.txt").await;

    let mod_path = dir.path().join(MODS_SUBPATH).join(mod_id);
    // Simulate Playlunky writing savegame data.
    touch_file(mod_path.join("save.dat")).await;
    touch_file(mod_path.join("savegame.sav")).await;
    touch_file(mod_path.join("save_main.dat")).await;
    // A non-savegame file that should be swept away by the update.
    touch_file(mod_path.join("ephemeral.txt")).await;

    // Update from a different source to prove the extraction actually
    // ran (single_file.zip contains lonely.txt).
    update_from_local_sources(&handle, "single_file.zip", mod_id).await;

    assert_exits_in(&dir, mod_id, "save.dat").await;
    assert_exits_in(&dir, mod_id, "savegame.sav").await;
    assert_exits_in(&dir, mod_id, "save_main.dat").await;
    assert_exits_in(&dir, mod_id, "lonely.txt").await;
    assert_missing(&dir, mod_id, "ephemeral.txt").await;
    assert_missing(&dir, mod_id, "foo.txt").await;
}
