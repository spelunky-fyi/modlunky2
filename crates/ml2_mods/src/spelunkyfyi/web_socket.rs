use std::path::Path;
use std::time::Duration;

use anyhow::anyhow;
use derive_more::Debug;
use futures_util::{SinkExt as _, StreamExt as _};
use ml2_net::backoff::{AsBackoffKind, BackoffKind, RetryPolicy};
use rand::distr::Uniform;
use rand::{Rng, RngExt};
use serde::{Deserialize, Serialize};
use tokio::sync::watch;
use tokio::{net::TcpStream, select, time};
use tokio_graceful_shutdown::{IntoSubsystem, SubsystemHandle};
use tokio_tungstenite::tungstenite::http::{
    self, HeaderValue, Request, StatusCode, Uri,
    header::{AUTHORIZATION, CONNECTION, HOST, SEC_WEBSOCKET_KEY, SEC_WEBSOCKET_VERSION, UPGRADE},
    uri::{InvalidUri, InvalidUriParts},
};
use tokio_tungstenite::{
    MaybeTlsStream, WebSocketStream,
    tungstenite::{
        Error as WsError, Message, handshake::client::generate_key, protocol::WebSocketConfig,
    },
};
use tracing::{debug, error, info, instrument, trace, warn};

use crate::manager::{ModManagerHandle, ModSource};

use super::Error;

pub const DEFAULT_MIN_PING_INTERVAL: Duration = Duration::from_secs(15);
pub const DEFAULT_MAX_PING_INTERVAL: Duration = Duration::from_secs(25);
pub const DEFAULT_PONG_TIMEOUT: Duration = Duration::from_secs(10);

/// Lifecycle state the caller can observe via `with_status_channel`.
/// Frontends surface it as a small connection indicator so the user knows
/// whether the site's "Install in modlunky2" button will work.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ConnectionStatus {
    /// No active connection: never connected, upstream closed the socket,
    /// or the subsystem has been shut down.
    Disconnected,
    /// Trying to reach the site (initial connect or reconnect backoff).
    Connecting,
    /// WebSocket upgrade completed. Install actions from the site will
    /// route into the mod manager.
    Connected,
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
struct ChannelMessage {
    action: String,
    channel_name: String,
    data: Option<MessageData>,
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
struct MessageData {
    install_code: String,
    mod_file_id: String,
}

#[derive(Debug)]
enum Check {
    Ping(),
    Pong(),
}

#[derive(Debug)]
pub struct WebSocketClient {
    #[debug(skip)]
    authz_value: HeaderValue,
    manager_handle: ModManagerHandle,
    ping_interval_dist: Uniform<Duration>,
    pong_timeout: Duration,
    service_uri: Uri,
    retry_policy: RetryPolicy,
    /// Optional status channel. Every lifecycle transition sends the
    /// new state; consumers dedupe naturally via `watch`. Cheap when
    /// unset (a couple of `is_some` checks per retry loop iteration).
    #[debug(skip)]
    status_tx: Option<watch::Sender<ConnectionStatus>>,
}

impl WebSocketClient {
    pub fn new(
        service_root: &str,
        auth_token: &str,
        manager_handle: ModManagerHandle,
        ping_interval_dist: Uniform<Duration>,
        pong_timeout: Duration,
    ) -> Result<Self, Error> {
        Ok(WebSocketClient {
            authz_value: HeaderValue::from_str(&format!("Token {auth_token}"))?,
            manager_handle,
            ping_interval_dist,
            pong_timeout,
            retry_policy: RetryPolicy::new(),
            service_uri: root_to_service_uri(service_root)?,
            status_tx: None,
        })
    }

    /// Sink connection-status transitions into `tx`. Callers construct the
    /// watch channel with the initial state they want the UI to show while
    /// the client is spinning up.
    pub fn with_status_channel(mut self, tx: watch::Sender<ConnectionStatus>) -> Self {
        self.status_tx = Some(tx);
        self
    }

    fn set_status(&self, status: ConnectionStatus) {
        if let Some(tx) = &self.status_tx {
            let _ = tx.send(status);
        }
    }

    #[instrument(skip_all)]
    async fn try_connecting(&mut self, subsys: &SubsystemHandle) -> Result<(), ConnectionError> {
        let mut last_result = Ok(());
        loop {
            select! {
                _ = subsys.on_shutdown_requested() => break,
                () = self.retry_policy.wait_to_retry(last_result)? => {
                    self.set_status(ConnectionStatus::Connecting);
                    last_result = self.connect_and_run(subsys).await;
                    self.set_status(ConnectionStatus::Disconnected);
                    // This is complicated because a configuration problem
                    // should surface differently on an HTTP 403 Forbidden.
                    if let Err(e) = &last_result {
                        debug!("WebSocket connection error: {:?}", e);
                    }
                }
            }
        }
        Ok(())
    }

    #[instrument(skip_all)]
    async fn connect_and_run(&self, subsys: &SubsystemHandle) -> Result<(), ConnectionError> {
        debug!("Attempting to connect to {:?}", self.service_uri);
        // Tungstenite checks for various headers, but doesn't expose a
        // Builder or reusable constants. Build the Request explicitly so
        // the authz header can be set.
        let request = Request::get(&self.service_uri)
            .version(http::Version::HTTP_11)
            .header(HOST, self.service_uri.authority().unwrap().as_str())
            .header(AUTHORIZATION, &self.authz_value)
            .header(CONNECTION, "Upgrade")
            .header(UPGRADE, "websocket")
            .header(SEC_WEBSOCKET_VERSION, "13")
            .header(SEC_WEBSOCKET_KEY, generate_key())
            .body(())?;

        let mut config = WebSocketConfig::default();
        config.max_write_buffer_size = 2 * config.write_buffer_size;

        let (mut stream, _response) =
            tokio_tungstenite::connect_async_with_config(request, Some(config), false).await?;
        debug!("WebSocket connected");
        self.set_status(ConnectionStatus::Connected);
        self.handle_messages(subsys, &mut stream).await
    }

    async fn handle_messages(
        &self,
        subsys: &SubsystemHandle,
        stream: &mut WebSocketStream<MaybeTlsStream<TcpStream>>,
    ) -> Result<(), ConnectionError> {
        let mut check_state = Check::Ping();
        let mut check_sleep = Box::pin(time::sleep(rand::rng().sample(self.ping_interval_dist)));

        loop {
            select! {
                _ = subsys.on_shutdown_requested() => {
                    stream.close(None).await.map_err(|e| {
                        debug!("Error trying to close WebSocket: {:?}",  e);
                        e
                    })?;
                    break
                },
                () = (&mut check_sleep) => match check_state {
                    Check::Ping() => {
                        trace!("Time to send a ping");
                        let mut payload = vec![0_u8; 8];
                        rand::rng().fill_bytes(&mut payload[..]);
                        stream
                            .send(Message::Ping(payload.into()))
                            .await?;
                        stream.flush().await?;
                        check_state = Check::Pong();
                        check_sleep = Box::pin(time::sleep(self.pong_timeout));
                    }
                    Check::Pong() => {
                        debug!("Timed out waiting for WebSocket pong");
                        // Timed out, start reconnecting
                        return Ok(());
                    }
                },
                Some(msg) = stream.next() => match msg? {
                    Message::Ping(_) => {
                        // Responding Pong is enqueued by Tungstenite
                        trace!("Received ping from server");
                    },
                    Message::Pong(_) => {
                        trace!("Received a pong");
                        // Accept any pong. That's fine since the ping/pong
                        // is only keeping the connection alive, not
                        // measuring latency.
                        check_state = Check::Ping();
                        check_sleep =
                            Box::pin(time::sleep(rand::rng().sample(self.ping_interval_dist)));
                    },
                    Message::Close(_) => {
                        debug!("WebSocket closed");
                        // Responding Close is enqueued by Tungstenite
                        break;
                    },
                    Message::Text(json) => self.handle_json(stream, json.as_bytes()).await?,
                    Message::Binary(json) => self.handle_json(stream, &json[..]).await?,
                    Message::Frame(_) => {
                        warn!("Rececived unexpected Frame message from WebSocket");
                    },
                }
            }
        }
        Ok(())
    }

    async fn handle_json(
        &self,
        stream: &mut WebSocketStream<MaybeTlsStream<TcpStream>>,
        json: &[u8],
    ) -> Result<(), ConnectionError> {
        debug!(
            "Parsing JSON message {:?}",
            std::str::from_utf8(json)
                .ok()
                .unwrap_or("(UTF-8 conversion failed)")
        );

        let msg = match serde_json::from_slice::<ChannelMessage>(json) {
            Err(e) => {
                warn!("Failed to parse WebSocket message from server: {:?}", e);
                return Ok(());
            }
            Ok(msg) => msg,
        };

        match msg.action.as_str() {
            "web-connected" | "hello" => {
                debug!("Received channel greeting of type {:?}", msg.action);
                send_message(
                    stream,
                    ChannelMessage {
                        action: "announce".to_string(),
                        channel_name: msg.channel_name.clone(),
                        data: None,
                    },
                )
                .await?;
            }
            "web-disconnected" => {
                debug!("Received channel parting");
            }
            "install" => {
                info!("Received install request: {:?}", msg);
                self.handle_install(stream, msg).await?;
            }
            _ => {
                warn!(
                    "Received WebSocket message from server with unrecognized action {:?}",
                    msg.action
                );
            }
        }
        Ok(())
    }

    async fn handle_install(
        &self,
        stream: &mut WebSocketStream<MaybeTlsStream<TcpStream>>,
        msg: ChannelMessage,
    ) -> Result<(), ConnectionError> {
        let data = match msg.data {
            None => {
                warn!("Received install message without data");
                return Ok(());
            }
            Some(data) => data,
        };
        let res = self
            .manager_handle
            .install(&ModSource::Remote {
                code: data.install_code,
            })
            .await;
        if let Err(e) = res {
            // This error has nothing to do with the WebSocket
            warn!("Installing mod via WebSocket failed: {:?}", e);
        } else {
            send_message(
                stream,
                ChannelMessage {
                    action: "install-complete".to_string(),
                    channel_name: msg.channel_name.clone(),
                    data: None,
                },
            )
            .await?;
        }
        Ok(())
    }
}

impl IntoSubsystem<Error> for WebSocketClient {
    async fn run(mut self, subsys: &mut SubsystemHandle) -> Result<(), Error> {
        self.set_status(ConnectionStatus::Connecting);
        let result = self.try_connecting(&*subsys).await;
        // try_connecting always ends up at Disconnected via the loop's
        // trailing set_status, but re-assert here so a shutdown that
        // races a Connected upgrade still flips the icon back.
        self.set_status(ConnectionStatus::Disconnected);
        if let Err(err) = &result {
            match err {
                ConnectionError::WebSocket(WsError::Http(resp)) => {
                    if resp.status() == StatusCode::FORBIDDEN {
                        error!("WebSocket authorization failed. Incorrect token?")
                    } else {
                        warn!("WebSocket couldn't connect: {:?}", err)
                    }
                }
                _ => warn!("WebSocket couldn't connect: {:?}", err),
            }
        }
        Ok(())
    }
}

async fn send_message(
    stream: &mut WebSocketStream<MaybeTlsStream<TcpStream>>,
    msg: ChannelMessage,
) -> Result<(), ConnectionError> {
    let reply = serde_json::to_string(&msg)?;
    stream.send(Message::Text(reply.into())).await?;
    stream.flush().await?;
    Ok(())
}

fn root_to_service_uri(service_root: &str) -> Result<Uri, Error> {
    let service_root = service_root.parse::<Uri>()?;
    let scheme = match service_root.scheme_str().unwrap_or("ws") {
        "http" => "ws",
        "https" => "wss",
        "ws" => "ws",
        "wss" => "wss",
        _ => {
            return Err(Error::UnknownError(anyhow!(
                "Unknown service scheme {:?}",
                service_root.scheme()
            )));
        }
    };

    let path = Path::new(service_root.path()).join("ws/gateway/ml/");
    let path = path
        .to_str()
        .ok_or_else(|| Error::InvalidUri(anyhow!("Failed to convert {path:?}")))?;

    let mut parts = service_root.into_parts();
    parts.scheme = Some(scheme.try_into()?);
    parts.path_and_query = Some(path.try_into()?);
    Ok(Uri::from_parts(parts)?)
}

#[derive(Debug, thiserror::Error)]
enum ConnectionError {
    #[error(transparent)]
    Http(#[from] http::Error),
    #[error(transparent)]
    Json(#[from] serde_json::Error),
    #[error(transparent)]
    WebSocket(#[from] WsError),
}

impl From<ConnectionError> for Error {
    fn from(err: ConnectionError) -> Self {
        match err {
            ConnectionError::Http(inner) => inner.into(),
            ConnectionError::Json(inner) => inner.into(),
            ConnectionError::WebSocket(inner) => inner.into(),
        }
    }
}

impl From<InvalidUri> for Error {
    fn from(e: InvalidUri) -> Error {
        Error::InvalidUri(e.into())
    }
}

impl From<InvalidUriParts> for Error {
    fn from(e: InvalidUriParts) -> Error {
        Error::InvalidUri(e.into())
    }
}

impl From<http::Error> for Error {
    fn from(e: http::Error) -> Error {
        Error::GenericHttpError(e.into())
    }
}

impl AsBackoffKind for ConnectionError {
    fn as_backoff_kind(&self) -> BackoffKind {
        match self {
            // JSON decode / HTTP request-builder failures point at a
            // programming or configuration problem the client can't
            // recover from by retrying. Stop the loop.
            ConnectionError::Json(_) => BackoffKind::Permanent,
            ConnectionError::Http(_) => BackoffKind::Permanent,
            ConnectionError::WebSocket(inner) => match &inner {
                // HTTP status on upgrade: 4xx generally means the
                // credentials or URL are wrong; 5xx / rate-limit /
                // timeout are worth retrying.
                WsError::Http(resp) => match resp.status() {
                    StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN | StatusCode::NOT_FOUND => {
                        BackoffKind::Permanent
                    }
                    _ => BackoffKind::Transient,
                },
                // Config-shaped WsErrors that won't get better on a
                // reconnect. Everything else (Io, ConnectionClosed,
                // AlreadyClosed, Protocol, Capacity, Tls...) is
                // reachable during a normal server restart or brief
                // network hiccup and MUST retry.
                WsError::Url(_) | WsError::HttpFormat(_) => BackoffKind::Permanent,
                _ => BackoffKind::Transient,
            },
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use tokio_tungstenite::tungstenite::http::Response;

    fn ws_http_err(status: StatusCode) -> ConnectionError {
        ConnectionError::WebSocket(WsError::Http(Box::new(
            Response::builder().status(status).body(None).unwrap(),
        )))
    }

    #[test]
    fn connection_reset_retries() {
        // A server restart typically surfaces as one of these; all
        // three must be Transient so the retry loop keeps trying.
        let err = ConnectionError::WebSocket(WsError::ConnectionClosed);
        assert!(matches!(err.as_backoff_kind(), BackoffKind::Transient));
        let err = ConnectionError::WebSocket(WsError::AlreadyClosed);
        assert!(matches!(err.as_backoff_kind(), BackoffKind::Transient));
        let err = ConnectionError::WebSocket(WsError::Io(std::io::Error::new(
            std::io::ErrorKind::ConnectionReset,
            "server went away",
        )));
        assert!(matches!(err.as_backoff_kind(), BackoffKind::Transient));
    }

    #[test]
    fn auth_failures_are_permanent() {
        assert!(matches!(
            ws_http_err(StatusCode::UNAUTHORIZED).as_backoff_kind(),
            BackoffKind::Permanent,
        ));
        assert!(matches!(
            ws_http_err(StatusCode::FORBIDDEN).as_backoff_kind(),
            BackoffKind::Permanent,
        ));
    }

    #[test]
    fn transient_http_statuses_retry() {
        assert!(matches!(
            ws_http_err(StatusCode::SERVICE_UNAVAILABLE).as_backoff_kind(),
            BackoffKind::Transient,
        ));
        assert!(matches!(
            ws_http_err(StatusCode::TOO_MANY_REQUESTS).as_backoff_kind(),
            BackoffKind::Transient,
        ));
    }
}
