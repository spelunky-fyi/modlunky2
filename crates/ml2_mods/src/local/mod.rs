...
pub trait LocalMods {
    ...
    /// Lighter-weight update path fed by the batch check-updates endpoint,
    /// which returns just a slug -> newest ModFile map and never provides
    /// a full manifest. Semantics match `update_latest_json`: returns
    /// Some(id) if latest.json changed, None if it was already current.
    /// `id` is the same "fyi.<slug>" form used elsewhere.
