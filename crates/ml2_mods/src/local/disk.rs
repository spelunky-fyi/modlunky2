...
use super::{
    ...
    constants::{LATEST_FILENAME, MANIFEST_FILENAME, MOD_METADATA_SUBPATH, MODS_SUBPATH},
};
...
impl DiskMods {
    ...
    async fn finish_update(
        ...
        // Preserve save files across reinstalls. Any of these may exist
        // depending on which Playlunky save mode the mod uses.
        for filename in SAVE_FILENAMES {
            let temp_save_path = temp_mod_path.join(filename);
            if path_metadata(&temp_save_path).await?.is_some() {
                debug!("Restoring {filename}");
                fs::copy(&temp_save_path, self.mods_dir_path(dest_id).join(filename)).await?;
            }
        }
        Ok(r#mod)
    }
...
const SAVE_FILENAMES: &[&str] = &["save.dat", "savegame.sav", "save_main.dat"];
...
impl LocalMods for DiskMods {
    ...
    async fn update_latest_json(&self, api_mod: &ApiMod) -> Result<Option<String>> {
        let Some(mod_file) = api_mod.mod_files.first() else {
            return Ok(None);
        };
        self.apply_latest_check_result(&api_mod.slug, &mod_file.id)
            .await
    }
    ...
    async fn apply_latest_check_result(
        ...
        let id = format!("fyi.{slug}");
        let prev_latest = self.load_latest(&id).await?;
        let same = prev_latest.as_deref() == Some(mod_file_id);
        if same {
            return Ok(None);
        }
        debug!("Writing latest.json for {}", id);
        let json = serde_json::to_string(&LatestFile {
            id: mod_file_id.to_string(),
        })?;
    ...
    async fn get_mod_logo(&self, id: &str) -> Result<ModLogo> {
        ...
        // Match case-insensitively: a manifest may record the logo with an
        // upper/mixed-case extension (e.g. `icon.PNG`), which the old launcher
        // accepted. Without this the mod silently shows no logo.
...
fn paths_have_same_prefix(paths: &[PathBuf]) -> bool {
    ...
    for p in paths {
        ...
        if let Some(pre) = c {
            // Ignore paths that contain mod stuff.
...
#[cfg(test)]
mod tests {
    // Zip archive extraction path munging: `count_lua_paths`,
    // `paths_have_same_prefix`, and `fix_zip_names` compose to what
    // `extract_zip_archive` does after reading the wire names, so they
    // are exercised together against the expected paths. Info-ZIP
    // `0x7075` unicode filename handling is deferred because the `zip`
    // crate resolves those transparently at `enclosed_name()` time.
    use super::{count_lua_paths, fix_zip_names, paths_have_same_prefix};
    use std::path::PathBuf;

    fn simulate(in_paths: &[&str]) -> Vec<String> {
        let paths: Vec<PathBuf> = in_paths.iter().map(PathBuf::from).collect();
        let rename_lua = count_lua_paths(&paths) == 1;
        let remove_first = paths_have_same_prefix(&paths);
        fix_zip_names(paths, rename_lua, remove_first)
            .into_iter()
            .map(|p| p.to_string_lossy().replace('\\', "/"))
            .collect()
    }

    fn assert_paths(in_paths: &[&str], expected: &[&str]) {
        assert_eq!(simulate(in_paths), expected);
    }

    // Single-lua rename to main.lua (three sub-cases).
    #[test]
    fn zip_rename_to_main_lua_bare_single_file() {
        assert_paths(&["foo.lua"], &["main.lua"]);
    }
    #[test]
    fn zip_rename_to_main_lua_wrapped_single_file() {
        assert_paths(&["foo/foo.lua"], &["main.lua"]);
    }
    #[test]
    fn zip_rename_to_main_lua_two_lua_files_skips_rename() {
        // Two .lua entries -> rename_lua is false; different prefixes
        // -> no common-dir strip. Paths pass through unchanged.
        assert_paths(&["foo/foo.lua", "bar.lua"], &["foo/foo.lua", "bar.lua"]);
    }

    // Data / soundbank prefixes must never be unwrapped (two sub-cases).
    #[test]
    fn zip_data_prefix_never_stripped() {
        assert_paths(&["Data/foo"], &["Data/foo"]);
    }
    #[test]
    fn zip_soundbank_prefix_never_stripped() {
        assert_paths(&["soundbank/foo"], &["soundbank/foo"]);
    }

    // Unwrap a shared parent directory (two sub-cases).
    #[test]
    fn zip_common_dir_stripped_when_shared() {
        assert_paths(&["foo/test.png", "foo/bar.png"], &["test.png", "bar.png"]);
    }
    #[test]
    fn zip_common_dir_not_stripped_when_mixed() {
        // Third path breaks the shared prefix; leave everything alone.
        assert_paths(
            &["foo/test.png", "foo/bar.png", "test"],
            &["foo/test.png", "foo/bar.png", "test"],
        );
    }

    // Regression: standard mod layout at the archive root: main.lua +
    // mod_info.json + custom_images/*. Different components => no
    // strip; only one lua but it's already `main.lua` so rename is a
    // no-op.
    #[test]
    fn zip_bat_frend_regression_standard_layout_unchanged() {
        assert_paths(
