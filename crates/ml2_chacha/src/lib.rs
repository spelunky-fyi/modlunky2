...
mod tests {
    use crate::{
        NasamGenerator, Spel2ChaCha, Spel2ChaChaVersion1, Spel2ChaChaVersion2, bytes_to_words,
        mix_in, mix_in_filepath, quad_rounds, quarter_round, round_pair, words_to_bytes, xnasamx,
    };
    ...
    fn test_hash_filepath() {
        ...
        let mut nasam = NasamGenerator::default();
        let key = nasam.update(10);
        let chacha = Spel2ChaChaVersion2::new(key);
        let out = chacha.hash_filepath(b"soundbank.bank");
        assert_eq!(
            out,
            vec![
                195, 155, 109, 179, 10, 55, 151, 210, 190, 15, 107, 118, 50, 62
            ]
        );
    }
