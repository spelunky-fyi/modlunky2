...
/// Basenames of every known vanilla `Data/Textures/**/*.png` (e.g.
/// `"monsters_olmec.png"`, `"char_blue.png"`). Lets callers tell whether a mod
/// file replaces a known vanilla texture, which is useful for excluding
/// non-character textures from character detection.
pub fn known_texture_png_names() -> impl Iterator<Item = &'static str> {
    KNOWN_FILEPATHS.iter().filter_map(|p| {
        let s = std::str::from_utf8(p).ok()?;
        if !s.starts_with("Data/Textures/") || !s.ends_with(".png") {
            return None;
        }
        s.rsplit('/').next()
    })
