...
from .utils import Peekable, DirectivePrefixes, split_comment
        ...
        with level_path.open("r") as level_fh:
            ...
            for line in level_fh:
                ...
                if line.startswith(DirectivePrefixes.LEVEL_SETTING.value):
                    ...
                    # `-` is commonly used for section delimiters
                    stripped_comment = comment.strip("-")
                    if stripped_comment and file_comment is None and not seen_directive:
                        level_file.comment = comment
