...
class TextureUtil:
    def __init__(self, sprite_fetcher):
        ...
        self.draw_mode["anubis"] = 7
        self.draw_mode["olmec"] = 15
        self.draw_mode["alienqueen"] = 7
        self.draw_mode["kingu"] = 18
        self.draw_mode["coffin"] = 2
        self.draw_mode["dog_sign"] = 2
        self.draw_mode["bunkbed"] = 2
        self.draw_mode["telescope"] = 2
        self.draw_mode["palace_table"] = 11
        self.draw_mode["palace_chandelier"] = 11
        self.draw_mode["moai_statue"] = 9
        self.draw_mode["mother_statue"] = 10
        self.draw_mode["empress_grave"] = 2
        self.draw_mode["empty_mech"] = 2
        self.draw_mode["olmecship"] = 7
        self.draw_mode["lavamander"] = 2
        self.draw_mode["mummy"] = 2
        self.draw_mode["yama"] = 2
        self.draw_mode["crown_statue"] = 7
        self.draw_mode["lamassu"] = 2
        self.draw_mode["madametusk"] = 2
        self.draw_mode["giant_frog"] = 17
        self.draw_mode["door"] = 13
        self.draw_mode["starting_exit"] = 13
        self.draw_mode["eggplant_door"] = 13
        self.draw_mode["door2"] = 6
        self.draw_mode["door_drop_held"] = 6
        self.draw_mode["palace_entrance"] = 6
        self.draw_mode["door2_secret"] = 6
        self.draw_mode["ghist_door2"] = 6
        self.draw_mode["ghist_door"] = 2
        self.draw_mode["minister"] = 2
        self.draw_mode["storage_guy"] = 2
        self.draw_mode["idol"] = 4
        self.draw_mode["idol_held"] = 4
        self.draw_mode["ankh"] = 4
        self.draw_mode["fountain_head"] = 13
        self.draw_mode["yama"] = 14
        self.draw_mode["plasma_cannon"] = 4
        self.draw_mode["lockedchest"] = 4
        self.draw_mode["shopkeeper_vat"] = 12
        self.draw_mode["totem_trap"] = 2
        self.draw_mode["lion_trap"] = 2
        self.draw_mode["vlad"] = 4
        self.draw_mode["yeti_queen"] = 2
        self.draw_mode["yeti_king"] = 2
        self.draw_mode["crabman"] = 2
        self.draw_mode["giant_fly"] = 2
        self.draw_mode["ammit"] = 16
        self.draw_mode["humphead"] = 13

    def adjust_texture_xy(self, width, height, tile_name, scale=50):
        mode = self.draw_mode.get(tile_name)
    ...
    def get_texture(self, tile, biome, floor_biome, border_biome, lvl, scale):
        def get_specific_tile(tile):
            ...
            if (
                ...
                if tile == "floor":
                    img_spec = self.sprite_fetcher.get(
                        "generic_floor", str(floor_biome or biome)
                    )
            ...
            if (
                lvl.startswith("duat") and hard_floor_biome is None
            ) or hard_floor_biome == BIOME.DUAT:  # specific floor hard for this biome
                if tile == "floor_hard":
            ...
            if (
                lvl.startswith("babylon") and hard_floor_biome is None
            ) or hard_floor_biome == BIOME.NEO_BABYLON:
                if tile == "floor_hard":
        ...
        img = self.sprite_fetcher.get(
            str(tile),
            str(biome),
            floor_biome and str(floor_biome),
            border_biome and str(border_biome),
        )
        ...
        if len(tile.split("%", 2)) > 1:
            ...
            if self.sprite_fetcher.get(
                primary_tile,
                str(biome),
                floor_biome and str(floor_biome),
                border_biome and str(border_biome),
            ):
                img1 = self.sprite_fetcher.get(
                    primary_tile,
                    str(biome),
                    floor_biome and str(floor_biome),
                    border_biome and str(border_biome),
                )
                specific_tile = get_specific_tile(str(primary_tile))
            ...
            if len(tile.split("%", 2)) > 2:
                ...
                if self.sprite_fetcher.get(
                    ...
                    img2 = self.sprite_fetcher.get(
                        secondary_tile,
                        str(biome),
                        floor_biome and str(floor_biome),
                        border_biome and str(border_biome),
                    )
                    specific_tile = get_specific_tile(str(secondary_tile))
                    if specific_tile is not None:
                        img2 = specific_tile
